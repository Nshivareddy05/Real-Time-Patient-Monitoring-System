...
def predict(x):
    with torch.no_grad():
        ...
        prob = output.item()
        return ("LOW RISK" if prob > 0.5 else "HISK RISK", prob)
...
demo_mode = st.sidebar.selectbox("Menu", ["Dashboard", "High Risk Samples", "Low Risk Samples", "Real Time Values", "Project Details"])
    ...
    if len(data_class_1) == 0:
        ...
        if st.session_state.simulate_high_risk:
            ...
            for idx, sample in data_class_1.iterrows():
                ...
                prob_placeholder.metric("Risk Probability", f"{probability:.2%}")
